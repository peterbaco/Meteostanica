...
// --- OLED: hodinová história tlaku pre 3-hodinový trend (rovnaký princíp
// ako PressureMixin.update_forecast() v appke - okno = posledné 3 hodiny,
// vzorka raz za hodinu, teda 4 sloty vrátane oboch koncov okna). Appka má
...
// Adamov/Brno - trend (ZMENA tlaku za 3h) je ale na nadmorskej výške
...
const uint8_t PRESSURE_HISTORY_SIZE = 4;
...
// --- Zostaví krátky text trendu tlaku ("+0.3/3h Pekne" a pod.) do
...
// v config.py appky (6,0 / 1,6 hPa za 3h okno). ---
void getPressureTrendText(char *buffer, size_t bufferSize) {
  ...
  snprintf(buffer, bufferSize, "%+.1f/3h %s", change, label);
}
...
//   riadok 3 (y=24, veľkosť 1): trend za posledné 3h
...
void loop() {
  ...
  // WiFi je NEKRITICKÝ doplnok rovnako ako INA226/OLED - jej prípadný výpadok
  // nesmie zablokovať meranie/OLED, len sa dáta dočasne neodošlú na server
  // (ensureWifiConnected() sama throttluje pokusy o reconnect).
  ensureWifiConnected();
