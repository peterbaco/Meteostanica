...
def init_db():
    ...
    # Displej/senzory ESP32 - predvolene zapnuté (appka pri prvom štarte
    # ešte nič nevypla), pozri get_display_enabled/get_sensors_enabled nižšie.
    if get_setting("display_enabled") is None:
        set_setting("display_enabled", "1")
    if get_setting("sensors_enabled") is None:
        set_setting("sensors_enabled", "1")
...
# ----------------------------------------------------------------------
# Vzdialené vypnutie displeja/senzorov na ESP32 (napr. cez noc, aby nebolo
# treba fyzicky odpájať batériu) - appka len uloží ŽELANÝ stav sem, ESP32
# si ho pravidelne vyzdvihne cez GET /api/device-state (pozri main.py a
# pollDeviceState() v teplomer_I2C.ino). Uložené ako "1"/"0" string,
# rovnaký princíp ako ostatné nastavenia vyššie.
# ----------------------------------------------------------------------
def get_display_enabled():
    return get_setting("display_enabled", "1") == "1"


def get_sensors_enabled():
    return get_setting("sensors_enabled", "1") == "1"


def set_display_enabled(enabled):
    set_setting("display_enabled", "1" if enabled else "0")


def set_sensors_enabled(enabled):
    set_setting("sensors_enabled", "1" if enabled else "0")
...
def get_daily_averages(days=7):
    """Denné priemery + MIN/MAX teploty a vlhkosti (vrátane ČASU, kedy dané
    MIN/MAX v ten deň nastalo) za posledných `days` dní - ekvivalent karty
    'Posledných 7 dní' z Windows appky, rozšírené o časy výskytu (potrebné
    pre kruhové ukazovatele teploty/vlhkosti na hero stránke, pozri
    main.py/app.js). Skupinuje podľa lokálneho kalendárneho dňa
    ...
    with _cursor() as cur:
        ...
        # SQL agregácia vyššie vráti len SAMOTNÚ hodnotu MIN/MAX, nie kedy sa
        # namerala - dohľadáme to dodatočným dotazom na ts prvej vzorky s
        # danou hodnotou v danom dni. Obmedzené na rozsah kalendárneho dňa
        # (day_start..day_start+24h) namiesto strftime() nad celou tabuľkou,
        # aby dotaz mohol využiť index idx_readings_ts namiesto plného skenu.
        for row in rows:
            try:
                day_start = time.mktime(datetime.strptime(row["day"], "%Y-%m-%d").timetuple())
            except ValueError:
                continue
            day_end = day_start + 86400
            for value_key, ts_key, column in (
                ("min_temp", "min_temp_ts", "temp"),
                ("max_temp", "max_temp_ts", "temp"),
                ("min_hum", "min_hum_ts", "hum"),
                ("max_hum", "max_hum_ts", "hum"),
            ):
                row[ts_key] = None
